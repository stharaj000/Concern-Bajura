      ...
      <div className="mt-10">
        {data.timeline.map((entry) => (
          <div key={entry.year} className="flex gap-6">
            <div className="w-16 md:w-20 shrink-0 font-bold text-lg text-[var(--color-primary)]">
              {entry.year}
            </div>
            <div className="flex-1 border-l-2 border-[var(--color-border)] pl-6 pb-8 space-y-2.5">
              {entry.events.map((item, i) => (
                <div key={i} className="relative">
                  <span className="absolute -left-[29px] top-1.5 w-2.5 h-2.5 rounded-full bg-[var(--color-background)] border-2 border-[var(--color-primary)]" />
