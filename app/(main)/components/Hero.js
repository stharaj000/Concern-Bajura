export default function Hero({ data }) {
  return (
    <section className=" text-white h-screen relative flex">
      ...
      <div className="bg-white/10 rounded-2xl h-1/3 md:h-screen md:h-full w-full flex justify-center top-0 absolute z-10">
        <video autoPlay loop muted controls controlsList="nodownload noplaybackrate" playsInline className="w-screen brightness-50 h-full inset-0 object-cover" >
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-14 md:py-20 grid md:grid-cols-2 gap-10 items-center relative top-26 md:top-12 z-20">
        {/* left side - text content */}
        <div className="md:flex flex-col gap-4">
          <h1 className="text-2xl md:text-5xl font-bold leading-tight tracking-[-0.02em] text-text md:text-white md:text-shadow-black md:text-shadow-md">
            {data.title}
          </h1>
          ...
          <div className="mt-7 flex md:flex-wrap gap-4">
            <button className="bg-primary text-shadow-md text-white font-medium px-2 md:px-6 py-3 rounded-lg hover:bg-primary-hover cursor-pointer transition-colors">
              {data.button1Text}
            </button>
